...
class LSTMTestCase(unittest.TestCase):
    ...
    def test_init_build(self):
        ...
        name = "lstm_1"
        lstm_1 = LSTMMultiLayer(units_per_cell, batch_size, time_steps, n_features, n_layers=1, is_sequence_output=True,
                      n_output=n_output, f_out="identity", optimizer_name="rmsprop",
        ...
        outputs = lstm_1.model
        self.assertIsInstance(outputs, list)
        self.assertEqual(len(outputs), time_steps)
        self.assertTupleEqual(tuple(outputs[-1].get_shape().as_list()), (None, n_output))

        name = "lstm_2"
        lstm_2 = LSTMMultiLayer(units_per_cell, batch_size, time_steps, n_features, n_layers=1, is_sequence_output=False,
                      n_output=n_output, f_out="identity", optimizer_name="rmsprop",
                      learning_rate=0.1, epochs=1, from_pretrained=False, validation_step=1000,
                      checkpoint_step=1000, summary_path=os.path.join(summary_path, name),
                      checkpoint_path=os.path.join(checkpoint_path, name), name=name)

        outputs = lstm_2.model
        self.assertIsInstance(outputs, list)
    ...
    def test_fit(self):
        ...
        name = "lstm_1"
        lstm_1 = LSTMMultiLayer(units_per_cell, batch_size, time_steps, n_features, n_layers=1, is_sequence_output=False,
                      n_output=n_output, f_out="identity",  optimizer_name="rmsprop",
